...
(defn query-organizations [{:keys [organization
                                   state
                                   city
                                   postal-code
  ...
  (sql/format (->
               ...
               (where (cond-> [:and [:= :deleted false]]
                        ...
                        organization (conj (name-check-organization organization))
...
(defn check-condition [data]
  (let [params (assoc data :speciality (search-t-codes (:speciality data)))]
    (log/info params)
      (if (and (not (empty? (select-keys params [:first-name :last-name])) )
        ...
        (cond 
          ...
          (contains?  params :organization) (wrap-query-organizations params)
          :else (merge (wrap-query-practitioner params)
...
;; -> How search works
;; - `GET /search`
;; - `GET /search?speciality=ambulance`
...
;; - "`organization` searches organizations where `resource#>>'{name}`"
